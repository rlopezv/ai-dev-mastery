...
Persistent storage for audit reports generated by `/audit-doc`, `/audit-module`, `/audit`, and `/enrich`.
These files are committed to git and updated as audit phases complete.

### meta/session/reports/

Ephemeral cache for review reports generated by `/review-doc`. Consumed and deleted by `/fix-doc`.
This folder is excluded from git — see `.gitignore`. Reports do not survive across sessions.
...
- `meta/session/reports/` must not be committed to git
